...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-e", "--env", default="VSSTuning-v0", help="Name of the gym environment"
    )
    ...
    val_env = gym.wrappers.RecordVideo(
        gym.make("VSSVal-v0"), "videos/", episode_trigger=lambda x: True
    )
                    ...
                    if n_grads % 50000 == 0:
                        ...
                        for key in val_infos[0].keys():
                            if key == "TimeLimit.truncated":
                                continue
                            metrics[f"validation/{key}"] = np.mean(
                                [info[key] for info in val_infos]
                            )
