...
if __name__ == "__main__":
    ...
    parser.add_argument("-c_gk", "--checkpoint_gk", default=None,
                        required=True)
    ...
    # Lendo o goleiro treinado
    checkpoint_gk=torch.load(args.checkpoint_gk)

    if 'state_dict_act' in checkpoint_gk:
        pi_gk=DDPGActor(40, 2).to(device)
        pi_gk.load_state_dict(checkpoint_gk['state_dict_act'])
    else:
        pi_gk= DDPGActor(checkpoint_gk['N_OBS'], checkpoint_gk['N_ACTS']).to(device)
        pi_gk.load_state_dict(checkpoint_gk['pi_state_dict'])
    pi_gk.eval()

    # Lendo a politica a ser treinada
