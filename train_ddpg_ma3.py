...
if __name__ == "__main__":
    ...
    Q = DDPGCritic(hp.N_OBS, hp.N_ACTS * 3).to(device)
    ...
    buffer = ReplayBuffer(buffer_size=hp.REPLAY_SIZE,
                          observation_space=gym.spaces.Box(low=hp.observation_space.low[0], high=hp.observation_space.high[1], shape=(3, hp.N_OBS), dtype=hp.observation_space.dtype),
                          action_space=gym.spaces.Box(low=-1, high=1, shape=(3, 2), dtype=np.float32),
                          device=hp.DEVICE
                          )
    n_grads = 0
    n_samples = 0
    ...
    try:
        while n_grads < hp.TOTAL_GRAD_STEPS:
            ...
            # train critic
            Q_opt.zero_grad()
            Q_v = Q(S_v[:, 0], A_v)  # expected Q for S,A
            A_next_v = tgt_pi(S_next_v)  # Get an Bootstrap Action for S_next
            Q_next_v = tgt_Q(S_next_v[:, 0], A_next_v.reshape(-1,6))  # Bootstrap Q_next
            ...
            pi_opt.zero_grad()
            A_cur_allies = pi(S_v[:,1:]).detach()
            A_cur_v = pi(S_v[:,:1])
            pi_loss_v = -Q(S_v[:,0], torch.cat((A_cur_v, A_cur_allies), 1).reshape(-1,6))
